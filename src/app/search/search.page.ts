  ...
  ionViewWillEnter() {
    ...
    if (inputs.type === "houses") {
      ...
      this.provider.queryData('houses', 'category', parseInt(inputs.search.category)).then(res => {
        this.data['result'] = [];
        res.forEach(doc => {
          this.data.result.push(doc.data());
        })
        //filter the search here and proceed
        this.data.result = this.data.result.filter(res => res.name.toLowerCase().includes(inputs.search.input.toLowerCase()));
