...
export class ProfilePage implements OnInit {
  ...
  saveAll = () => {
    ...
    this.action.post('/api/api/add_image.php', body).then(res => {
      this.data['r'] = res;
      if (this.data.r.data.message === 'success') {
        ...
        if (this.fire.save(data, 'verification/' + data.user)) {
  ...
  add_image = (ev) => {
    ...
    let res = this.action.processSingleImage(ev).then(res => {
      let props = {
        header: 'Confirm',
        ...
        buttons: [
          ...
          },
          {
            ...
            handler: () => {
              ...
              this.fire.uploadFile(res, metadata, n).then(res => {
